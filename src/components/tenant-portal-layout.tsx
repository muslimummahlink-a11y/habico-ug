  ...
  const { data: lease } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await supabase
        .from("leases")
        .select("*, units!inner(unit_number, floor_number, monthly_rent, properties!inner(name, location, owner_id))")
          ...
          {(lease || outstanding !== undefined) && (
            <div className="grid gap-4 md:grid-cols-3 mb-6">
              <Card className="shadow-card">
                ...
                <CardContent>
                  <p className={`text-2xl font-bold ${(outstanding ?? 0) > 0 ? "text-red-500" : "text-green-500"}`}>
                    UGX {(outstanding ?? 0).toLocaleString()}
                  </p>
                  <p className="text-xs text-muted-foreground">Outstanding rent</p>
                </CardContent>
              </Card>
              <Card className="shadow-card">
                <CardHeader className="pb-2">
                ...
                <CardContent>
                  <p className="text-2xl font-bold">UGX {Number(lease?.monthly_rent ?? 0).toLocaleString()}</p>
                  <p className="text-xs text-muted-foreground">Due {lease?.payment_due_day ? `every ${lease.payment_due_day}th` : "monthly"}</p>
                </CardContent>
              </Card>
              <Card className="shadow-card">
                <CardHeader className="pb-2">
                ...
                <CardContent>
                  <p className="text-2xl font-bold">
                    {lease?.units?.unit_number ? `Unit ${lease.units.unit_number}` : "—"}
                  </p>
